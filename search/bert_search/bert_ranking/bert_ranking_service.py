# services/bert_ranking_service.py
...
# Import the new, dedicated BERT ranking handler
...
class BertRankRequest(BaseModel):
    query_vector: List[List[float]] # Expects a 2D array from the query processor
...
async def rank_bert(dataset_name: str, request: BertRankRequest, db: DatabaseHandler = Depends(get_db_handler)):
    """
    Handles ranking using a pre-computed dense document matrix.
    """
    try:
        handler = BertRankingHandler(dataset_name, db_handler=db)
        # The rank method now takes the query vector directly
